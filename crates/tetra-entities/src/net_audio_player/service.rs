...
impl AudioPlayerHandle {
    ...
    /// Queue a complete WAV generated by the local TTS service.
    ///
    /// The TTS cache file is kept by `TtsHandle` until the audio-player job has
    /// finished preparing and transmitting it.
    pub fn play_generated_audio(
        ...
        self.play_resolved(
            ResolvedAudioSource {
                path: canonical,
                display_name,
                source_type: AudioSourceType::Tts,
                source_id: Some("tts".to_string()),
                cache_before_decode: false,
            },
            target_type,
            target_id,
