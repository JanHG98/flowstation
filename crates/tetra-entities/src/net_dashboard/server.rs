...
/// Body examples:
///   {"target":"secondary", "enabled": true}  -> enables secondary AND third when configured.
///   {"target":"secondary", "enabled": false} -> disables secondary AND third.
///   {"target":"third", "enabled": true}       -> enables only the third carrier, requires secondary ON.
///   {"target":"third", "enabled": false}      -> disables only the third carrier.
///
...
fn serve_dual_carrier_post(
    ...
    let target = req
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("secondary");
    if !matches!(target, "secondary" | "third") {
        return http_response(stream, 400, "target must be 'secondary' or 'third'");
    }

    let Some(requested_enabled) = req.get("enabled").and_then(|v| v.as_bool()) else {
    ...
    let mut secondary_enabled = current.enabled;
    let mut secondary_carrier = current.secondary_carrier;
    let mut third_enabled = current.third_enabled;
    let mut third_carrier = current.third_carrier;

    let requested_secondary = match parse_carrier("secondary_carrier") {
        Ok(v) => v,
    ...
    match target {
        "secondary" => {
            secondary_enabled = requested_enabled;
            if requested_enabled {
                secondary_carrier = requested_secondary.or(secondary_carrier);
                if secondary_carrier.is_none() {
                    return http_response(
                        stream,
                        400,
                        "enabling traffic carrier needs secondary_carrier in config.toml",
                    );
                }

                // Jan's lab behaviour: switching the 2nd/traffic carrier ON also arms the
                // configured 3rd carrier automatically. The 3rd can still be switched OFF again
                // with its own button after the restart.
                third_carrier = requested_third.or(third_carrier);
                third_enabled = third_carrier.is_some();
            } else {
                // The third carrier depends on the second traffic carrier being active.
                third_enabled = false;
            }
        }
        "third" => {
            third_carrier = requested_third.or(third_carrier);
            if requested_enabled {
                if !current.secondary_active() && !secondary_enabled {
                    return http_response(stream, 400, "enable the traffic carrier before enabling third carrier");
                }
                if secondary_carrier.is_none() {
                    return http_response(stream, 400, "third carrier requires a configured secondary_carrier");
                }
                if third_carrier.is_none() {
                    return http_response(stream, 400, "enabling third carrier needs third_carrier in config.toml");
                }
                // Keep secondary enabled when the operator enables the third carrier.
    ...
    let prospective = dual_carrier::compute_toml(
        &original,
        secondary_enabled,
        secondary_carrier,
        third_enabled,
    ...
    if let Err(e) = dual_carrier::write_dual_carrier(
        config_path,
        secondary_enabled,
        secondary_carrier,
        third_enabled,
    ...
    tracing::info!(
        "Dashboard: Carrier toggle target={} secondary={} ({:?}), third={} ({:?}); scheduling restart",
        target,
        if secondary_enabled { "ON" } else { "OFF" },
        secondary_carrier,
    ...
    let response = match target {
        "secondary" if secondary_enabled => {
            "Traffic carrier enabled; third carrier was armed automatically when configured. Restarting."
        }
        "secondary" => "Traffic carrier disabled; third carrier disabled as dependency. Restarting.",
        "third" if third_enabled => "Third carrier enabled. Restarting.",
        "third" => "Third carrier disabled. Restarting.",
        _ => "Carrier state updated. Restarting.",
    };
    http_response(stream, 200, response);
}
