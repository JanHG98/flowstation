...
async function refreshAll(){try{const [s,n,e]=await Promise.all([getj('/api/v1/status'),getj('/api/v1/nodes'),getj('/api/v1/events?limit=40')]);document.getElementById('cards').innerHTML=[card('Verbunden',s.connected_nodes),card('Bekannt',s.known_nodes),card('Stale',s.stale_nodes),card('Backend-Clients',s.backend_clients),card('Nachrichten',s.total_node_messages),card('Kommandos',s.total_commands)].join('');document.getElementById('nodes').innerHTML=n.map(x=>`<tr><td>${statusPill(x)}</td><td><b>${esc(x.identity.station_name)}</b><br><span class="small">${esc(x.node_id)}<br>${esc(x.peer)}</span></td><td>MCC ${esc(x.identity.mcc)} / MNC ${esc(x.identity.mnc)}<br>LA ${esc(x.identity.location_area)}, CC ${esc(x.identity.colour_code)}<br>Carrier ${esc(x.identity.main_carrier)}${x.identity.secondary_carrier?` / ${esc(x.identity.secondary_carrier)}`:''}</td><td>${esc(x.identity.stack_version)}</td><td>${esc(x.last_seen)}<br><span class="small">${esc(x.last_message_kind)}</span></td><td>Msg ${esc(x.message_count)}<br>Tel ${esc(x.telemetry_count)}<br>Ack ${esc(x.control_ack_count)}</td><td class="small">${esc(caps(x.capabilities))}</td><td><button onclick="action('${esc(x.node_id)}','ping')">Ping</button><button class="danger" onclick="action('${esc(x.node_id)}','disconnect')">Trennen</button></td></tr>`).join('')||'<tr><td colspan="8">Noch keine TBS verbunden.</td></tr>';document.getElementById('events').textContent=e.map(x=>`${x.timestamp} #${x.seq} ${x.kind}${x.node_id?' ['+x.node_id+']':''} ${JSON.stringify(x.detail)}`).join('\n')||'Noch keine Ereignisse.'}catch(e){document.getElementById('events').textContent='Fehler: '+e.message}}
